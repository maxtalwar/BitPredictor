...
def predict (train = 'prices.csv', predict ='predict.csv', features = []):
    ...
    model = tc.boosted_trees_classifier.create(train_data, target='CHANGE', features=f, validation_set='auto', max_iterations = 25, max_depth = 10, verbose = False)
    ...
    info = [results['accuracy'], results['precision']]
